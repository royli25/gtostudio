...
static PARALLEL_CHANCE_CALLS: AtomicU64 = AtomicU64::new(0);
static PARALLEL_CHANCE_ITEMS: AtomicU64 = AtomicU64::new(0);
static PARALLEL_ACTION_CALLS: AtomicU64 = AtomicU64::new(0);
static PARALLEL_ACTION_ITEMS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_CALLS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_ITEMS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_CHANCE_CALLS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_CHANCE_ITEMS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_ACTION_CALLS: AtomicU64 = AtomicU64::new(0);
static SEQUENTIAL_ACTION_ITEMS: AtomicU64 = AtomicU64::new(0);
static MAX_PARALLEL_WIDTH: AtomicU64 = AtomicU64::new(0);
static MAX_SEQUENTIAL_WIDTH: AtomicU64 = AtomicU64::new(0);
...
/// Resets traversal counters used to diagnose Rayon work distribution.
pub fn reset_traversal_stats() {
    PARALLEL_CALLS.store(0, Ordering::Relaxed);
    PARALLEL_ITEMS.store(0, Ordering::Relaxed);
    PARALLEL_CHANCE_CALLS.store(0, Ordering::Relaxed);
    PARALLEL_CHANCE_ITEMS.store(0, Ordering::Relaxed);
    PARALLEL_ACTION_CALLS.store(0, Ordering::Relaxed);
    PARALLEL_ACTION_ITEMS.store(0, Ordering::Relaxed);
    SEQUENTIAL_CALLS.store(0, Ordering::Relaxed);
    SEQUENTIAL_ITEMS.store(0, Ordering::Relaxed);
    SEQUENTIAL_CHANCE_CALLS.store(0, Ordering::Relaxed);
    SEQUENTIAL_CHANCE_ITEMS.store(0, Ordering::Relaxed);
    SEQUENTIAL_ACTION_CALLS.store(0, Ordering::Relaxed);
    SEQUENTIAL_ACTION_ITEMS.store(0, Ordering::Relaxed);
    MAX_PARALLEL_WIDTH.store(0, Ordering::Relaxed);
    MAX_SEQUENTIAL_WIDTH.store(0, Ordering::Relaxed);
}

/// Returns traversal counters in a stable order for WASM diagnostics.
///
/// Order:
/// parallel calls/items, parallel chance calls/items, parallel action calls/items,
/// sequential calls/items, sequential chance calls/items, sequential action calls/items,
/// max parallel width, max sequential width.
pub fn traversal_stats() -> [u64; 14] {
    [
        PARALLEL_CALLS.load(Ordering::Relaxed),
        PARALLEL_ITEMS.load(Ordering::Relaxed),
        PARALLEL_CHANCE_CALLS.load(Ordering::Relaxed),
        PARALLEL_CHANCE_ITEMS.load(Ordering::Relaxed),
        PARALLEL_ACTION_CALLS.load(Ordering::Relaxed),
        PARALLEL_ACTION_ITEMS.load(Ordering::Relaxed),
        SEQUENTIAL_CALLS.load(Ordering::Relaxed),
        SEQUENTIAL_ITEMS.load(Ordering::Relaxed),
        SEQUENTIAL_CHANCE_CALLS.load(Ordering::Relaxed),
        SEQUENTIAL_CHANCE_ITEMS.load(Ordering::Relaxed),
        SEQUENTIAL_ACTION_CALLS.load(Ordering::Relaxed),
        SEQUENTIAL_ACTION_ITEMS.load(Ordering::Relaxed),
        MAX_PARALLEL_WIDTH.load(Ordering::Relaxed),
        MAX_SEQUENTIAL_WIDTH.load(Ordering::Relaxed),
    ]
}
...
pub(crate) fn for_each_child<T: GameNode, OP: Fn(usize) + Sync + Send>(node: &T, op: OP) {
    let actions = node.action_indices();
    let width = actions.len() as u64;

    if node.enable_parallelization() {
        PARALLEL_CALLS.fetch_add(1, Ordering::Relaxed);
        PARALLEL_ITEMS.fetch_add(width, Ordering::Relaxed);
        update_max(&MAX_PARALLEL_WIDTH, width);
        if node.is_chance() {
            PARALLEL_CHANCE_CALLS.fetch_add(1, Ordering::Relaxed);
            PARALLEL_CHANCE_ITEMS.fetch_add(width, Ordering::Relaxed);
        } else {
            PARALLEL_ACTION_CALLS.fetch_add(1, Ordering::Relaxed);
            PARALLEL_ACTION_ITEMS.fetch_add(width, Ordering::Relaxed);
        }
        actions.into_par_iter().for_each(op);
    } else {
        SEQUENTIAL_CALLS.fetch_add(1, Ordering::Relaxed);
        SEQUENTIAL_ITEMS.fetch_add(width, Ordering::Relaxed);
        update_max(&MAX_SEQUENTIAL_WIDTH, width);
        if node.is_chance() {
            SEQUENTIAL_CHANCE_CALLS.fetch_add(1, Ordering::Relaxed);
            SEQUENTIAL_CHANCE_ITEMS.fetch_add(width, Ordering::Relaxed);
        } else {
            SEQUENTIAL_ACTION_CALLS.fetch_add(1, Ordering::Relaxed);
            SEQUENTIAL_ACTION_ITEMS.fetch_add(width, Ordering::Relaxed);
        }
        actions.for_each(op);
    }
}
